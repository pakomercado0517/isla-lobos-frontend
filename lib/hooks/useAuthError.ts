 ...
 * @returns Una función `handleAuthError` que toma un error y devuelve una promesa que resuelve a `true` si fue un error de autenticación manejado, `false` en caso contrario.
 */
  ...
  const handleError = useCallback(
    async (error: unknown): Promise<boolean> => {
      ...
      if (AuthErrorHandler.requiresLogout(error)) {
        await AuthErrorHandler.handleAuthFailure(router, error);
