...
// Imports para futuras mejoras si se necesitan
// import { utcToZonedTime, zonedTimeToUtc } from 'date-fns';
// const MEXICO_TIMEZONE = 'America/Mexico_City';

/**
 * Interpreta una fecha YYYY-MM-DD del backend como fecha de México
 * y la convierte a Date object local para el cliente
 * 
...
export function interpretarFechaDelBackend(fechaYYYYMMDD: string): Date {
  try {
    ...
    // Según documentación, crear la fecha como si fuera en México
    // y luego convertir al timezone local del navegador
    const [año, mes, dia] = fechaYYYYMMDD.split('-').map(Number);
    
    // Crear fecha en México (medianoche del día especificado)
    const fechaEnMexico = new Date();
    fechaEnMexico.setFullYear(año, mes - 1, dia);
    fechaEnMexico.setHours(0, 0, 0, 0);
    
    // En la práctica, para fechas (sin horas), podemos simplemente
    // crear una fecha local que represente el mismo día
    const fechaLocal = new Date(año, mes - 1, dia);
    
    return fechaLocal;
  } catch (error) {
    console.error(`Error interpretando fecha del backend: ${fechaYYYYMMDD}`, error);
...
export function formatearFechaRegional(fechaYYYYMMDD: string): string {
  try {
    // Validar formato
    if (!/^\d{4}-\d{2}-\d{2}$/.test(fechaYYYYMMDD)) {
      console.warn(`Formato de fecha inválido: ${fechaYYYYMMDD}`);
      return fechaYYYYMMDD; // Fallback: mostrar string original
    }

    // Validar valores antes de crear la fecha
    const [año, mes, dia] = fechaYYYYMMDD.split('-').map(Number);
    if (año < 2020 || año > 2030 || mes < 1 || mes > 12 || dia < 1 || dia > 31) {
      console.warn(`Fecha fuera de rango esperado: ${fechaYYYYMMDD}`);
    ...
    console.error(`Error formateando fecha regional: ${fechaYYYYMMDD}`, error);
...
export function formatearFechaCompacta(fechaYYYYMMDD: string): string {
  try {
    ...
    console.error(`Error formateando fecha compacta: ${fechaYYYYMMDD}`, error);
    return fechaYYYYMMDD;
  }
}
...
export function obtenerFechaLocalYYYYMMDD(): string {
  try {
    ...
    console.error('Error obteniendo fecha local actual', error);
...
export function extraerFechaLocalYYYYMMDD(fecha: Date | string): string {
  try {
    ...
    console.error('Error extrayendo fecha local YYYY-MM-DD', error);
...
export function compararFechasYYYYMMDD(fecha1: string, fecha2: string): number {
  if (!esFechaValidaYYYYMMDD(fecha1) || !esFechaValidaYYYYMMDD(fecha2)) {
    console.warn('Fechas inválidas en comparación:', { fecha1, fecha2 });
    return 0;
  }
