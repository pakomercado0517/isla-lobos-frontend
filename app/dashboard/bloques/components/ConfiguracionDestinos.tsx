...
export function ConfiguracionDestinos({
  open,
  onOpenChange,
}: ConfiguracionDestinosProps) {
  ...
  const handleToggleBloques = async (
    destino: DestinoType,
    habilitar: boolean
  ) => {
    setLoading(true);
    try {
      ...
      clientLogger.error("Error al actualizar configuración:", error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Gestiona qué destinos utilizan bloques horarios. Puedes habilitar o
            deshabilitar el sistema de bloques para cada destino
            individualmente.
          </DialogDescription>
        </DialogHeader>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-2">
                      <h4 className="font-medium text-blue-900">
                        Sistema Híbrido de Bloques
                      </h4>
                      <p className="text-sm text-blue-700">
                        Cada destino puede funcionar con{" "}
                        <strong>bloques horarios</strong> (capacidad controlada)
                        o con <strong>horarios libres</strong> (solo especificar
                        hora). Isla de Lobos siempre requiere bloques.
                      </p>
                      <ul className="text-sm text-blue-600 space-y-1 ml-4">
                        <li>
                          • <strong>Con Bloques:</strong> Los prestadores deben
                          seleccionar un bloque disponible
                        </li>
                        <li>
                          • <strong>Sin Bloques:</strong> Los prestadores
                          especifican la hora de salida libremente
                        </li>
                      </ul>
                    </div>
                  </div>
                          ...
                          <CardDescription className="mt-1">
                            {config.destino === DESTINOS.ISLA_LOBOS && (
                              <span className="text-amber-600 font-medium">
                                🔒 Destino con bloques obligatorios (no se puede
                                deshabilitar)
                              </span>
                            )}
                          </CardDescription>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(checked) =>
                            handleToggleBloques(config.destino, checked)
                          }
                          disabled={
                            loading || config.destino === DESTINOS.ISLA_LOBOS
                          }
                        />
                      </div>

                      {config.bloques_habilitados &&
                        config.plantillas_count === 0 && (
                          <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                            <p className="text-sm text-yellow-800">
                              ⚠️ <strong>Sin plantillas configuradas.</strong>
                              Crea al menos una plantilla de bloque para que
                              este destino funcione correctamente.
                            </p>
                          </div>
                        )}
                    </CardContent>
